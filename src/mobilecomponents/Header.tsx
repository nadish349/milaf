...
export const Header = ({ showOnlyLogo = false, showNavigationWithoutShop = false }: HeaderProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    console.log('🛒 Header: Cart items changed:', cartItems.length, 'total items:', getTotalItems());
  ...
  useEffect(() => {
    const handleGuestCartUpdate = (e: CustomEvent) => {
      ...
      console.log('🛒 Mobile: Guest cart update event detected, updating temp cart count to:', totalItems);
